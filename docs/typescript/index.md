...
- 在默认的 strictPropertyInitialization 模式下(有配置文件)，属性是`必须初始化`的，如果没有初始话编译时就会报错(了解即可，目前没啥用)
...
- Omit<T, K> 从 T 中去除 K 属性 组成一个对象类型
...
- NonNullable`<T>` 去除 T 中的 null 或 undefined
