...
- 如果不使用转义符合，j son="{"name":"Alice"}" 那么每两个双引号会组队，结果就是 `"{"`、`":"`、`"}"`，不符合 json 规范
- 所以需要手动转义每个双引号 `"`，写起来比较麻烦。
...
- 这种方式实际是字符串拼接，可以分为以下步骤
  ...
  3. '"}' ->输出`"}`
  4. 然后这上面三个步骤拼接在一起就形成了`{"name": "Bob"}`

- 如果使用`""$name""` 这种形式， 那么`""` 和`""`会组成两对最终结果是`Bob`，没有双引号包裹，不符合 json 规范
